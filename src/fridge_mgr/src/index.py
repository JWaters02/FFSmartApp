...
from .inventory_utils import view_inventory, delete_entire_item, modify_items, modify_door_state,\
    generate_response, get_current_time_gmt, get_low_stock, update_desired_quantity
...
def handler(event, context):
    ...
    try:
        ...
        if action == "view_inventory":
            ...
            response = update_desired_quantity(table, pk, body)
            return response
        else:
...
def manage_inventory(table, pk, body, action):
    ...
    try:
        ...
        if action in ["add_item", "update_item"]:
            success, response = modify_items(item, body, action, current_time)
            if not success:
                return response
